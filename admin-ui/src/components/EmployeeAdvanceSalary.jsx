...
import { createTransactionForEmployeeAdvancePay } from '../services/transactionService'
import { useForm } from "react-hook-form"
import { zodResolver } from "@hookform/resolvers/zod"
import { ChevronLeft, LoaderCircle } from 'lucide-react'
import { toast } from 'react-toastify'
import { employeeAdvanceSalarySchema } from '../validations/employeeAdvanceSalarySchema'

function EmployeeAdvanceSalary({openForm, employee, onClose}) {
  ...
  const {
    ...
    mode: 'onChange',
    resolver: zodResolver(employeeAdvanceSalarySchema),
    defaultValues: {
  ...
  const onSubmit = async (formData) => {
    setLoader(true)
    try{
        const data = await createTransactionForEmployeeAdvancePay(formData)
        onClose(true)
        toast.success("Advance salary paid successfully.")
    }catch(err){
        console.error(err)
    ...
    <div className='fixed z-50 backdrop-blur-sm inset-0 bg-black/40 flex justify-center items-center px-4 py-4 sm:px-6 sm:py-6'>
        <div className='flex w-full max-w-xl flex-col gap-3 sm:gap-4 bg-white rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 max-h-[90vh] overflow-y-auto'>
          <div className="flex items-center gap-2">
           ...
           <h1 className="text-lg sm:text-xl md:text-2xl font-semibold break-words">Advance Salary Pay</h1>
          </div>
