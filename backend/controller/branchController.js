...
export const createBranch = async (req, res, next) =>{
    try{
        ...
        return res.status(201).json({message:"New branch created successfully",success:true})
...
export const getAllBranch = async (req, res, next) =>{
    try{
        ...
        let branch = []
        if(searchQuery){
          branch = await BRANCH.find({branch_name:{$regex:searchQuery, $options:'i'}}).sort({createdAt:-1})
        }else{
          branch = await BRANCH.find().sort({createdAt:-1})
        }

        return res.status(200).json({message:"All Branches retrived successfully.",success:true, data:branch})
    }catch(err){
        next(err)
    }
}
...
export const updateBranch = async (req, res, next) =>{
    try{
       ...
       return res.status(200).json({message:"Branch saved successfully.",success:true})
...
export const getBranchById = async (req, res, next) =>{
    try{
        ...
        if(!branch) return res.status(200).json({message:"Branch not found.",success:false})

        return res.status(200).json({message:"Branch details retrived successfully.",success:true,data:branch})
