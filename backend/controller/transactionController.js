...
export const getAllTransactions = async (req, res, next) => {
   try {
     ...
     if (bank_account) filter.bank_account = bank_account;
     ...
     for (const tx of transactions) {
       ...
       // Deposite + Rent both are customer based
       if (
         (tx.refModel === "Depositeamount" ||
          tx.refModel === "Rentattempt") &&
         tx.refId?.customer
       ) {
     ...
     const finalData = transactions.map((tx) => {
       ...
       switch (tx.refModel) {
         ...
         case "Monthlypaymentreceipt":
           // adjust this field name if different in your schema
